...
def get_status(resources):
    try:
        response_status = urllib.request.urlopen(resources[1], timeout=5).getcode()
    ...
    return accessible, resources[0]

def multiprocessing_requests(resources):
    p = multiprocessing.Pool(80)
    resources_values = p.map(get_status, resources)
    p.close()
    p.join()
